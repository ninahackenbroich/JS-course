...
sendbutton.addEventListener('click', function () {
  ...
  const singelLines = textArea.split(/\n/g);
  let ind = 1;

  for (const [i, line] of singelLines.entries()) {
    const [startWord, endWord] = line.trim().toLowerCase().split('_');

    const oneWord =
      startWord + endWord.replace(endWord[0], endWord[0].toUpperCase());
    console.log(oneWord.replace('_', ' ').padEnd(20) + `${'✅'.repeat(i + 1)}`);
  }
});
